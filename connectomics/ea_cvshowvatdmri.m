    ...
    keyboard
        ...
        if ~ret % something has changed since last time.
            ...
            if dimensionality % one of the vat checkboxes is active
                [~,thresh]=ea_cvshowfiberconnectivities(resultfig,fibersfile,seedfile,targetsfile,thresh,sides,options,stimparams,changedstates,'vat'); % 'vat' only used for storage of changes.
                if isstruct(handles)
